...
%%%
% message<two datasets: bpfix-test repair tasks with oracles for RQ1, bpfix-bench labeled rejections for RQ2 and RQ3>
% we use two datasets
% bpfix-test is 75 repair tasks, 40 proof-obligation and 35 from open-source projects (Cilium, xdp-tools, bpftime), each with an oracle
% bpfix-bench is the 235 reproduced rejections from section 2, each with a hand-assigned layer
% RQ1 runs on bpfix-test, RQ2 and RQ3 on bpfix-bench
...
% bpfix-test is a working suite, a frozen heldout split is future work
...
\noindent\textbf{Datasets.} We use two datasets. \testset holds 75 source-level repair tasks, 40 built around a verifier proof obligation and 35 minimized from open-source projects such as Cilium~\cite{cilium}, xdp-tools~\cite{xdptools}, and bpftime~\cite{zheng2025extending}, and each task ships the buggy program with an executable oracle. \bench holds the 235 reproduced rejections from \S\ref{sec:study:corpus}, each carrying a hand-assigned responsibility layer. RQ1 runs on \testset, and RQ2 and RQ3 run on \bench. 
% \xiangyu{Add reference to \testset and \bench. Additionally, do we have baseline to compare?} 
...
\noindent\textbf{RQ3: Layer attribution.} \sys reports the responsible layer as a best-effort heuristic (\S\ref{sec:design}). The \bench layer labels were developed alongside \sys, so we examine layer separation through a case study on one message that two layers share. The message \texttt{invalid mem access 'scalar'} prints in \bench for both source bugs and compiler-lowering rejections (\S\ref{sec:study}). We study one rejection of each kind, and the compiler-lowering rejection has correct source with an upstream fix that steers the compiler, so its layer holds independent of \sys.
