...
%% \subsection{\bench: A Labeled Rejection Corpus}

%%%
% message<we build a corpus of real rejections to study what developers hit>
% to study rejections developers actually hit, we assemble bpfix-bench
...
% 235 of 936 reproduce and form bpfix-bench (table)
...
%%%
% message<we label each case for the ground truth the rejection lacks>
% we label each case for the ground truth the rejection lacks
...
% a source bug, a compiler-lowering rejection, and an environment issue print the same line
% the figure pairs a real source bug with a compiler-lowering rejection (figure)
% the source bug is fixed in the program
% the compiler-lowering rejection is fixed by steering the compiler
% reading the wrong layer is costly: a misread compiler-lowering rejection chases a fix in the wrong layer
...
To study rejections that developers actually hit, we assemble \bench, a corpus of real verifier rejections reproduced in one fixed environment. The corpus draws on 936 candidate reports from four sources: Stack Overflow questions, GitHub issues, GitHub fix commits, and kernel selftests. To compare rejections on equal footing, we replay every candidate in one environment, rebuilding the program and loading it into kernel 6.15.11 with clang 18 at log level 2. We keep a candidate only when the verifier rejects the rebuilt program and the harness captures a log. As Table~\ref{tab:corpus-outcomes} reports, 235 of the 936 candidates reproduce and form \bench. The rest are dropped because they need a specific environment or are not verifier errors. Because reproducing a rejection favors the cases that rebuild easily, the corpus skews toward simpler rejections.

\input{tables/sec-2-corpus-outcomes}

We label each case to recover the ground truth the rejection does not carry. For every rejection we read the faulty source and its fix. From that reading we record four labels: the responsible layer, the violated safety property, the source mechanism, and the kind of fix. The responsible-layer labels agree with the corpus's prior labels for 95.7\% of cases. We treat these labels as reliable. The property and mechanism labels are a first pass that still needs an independent reliability check.

\subsection{Characterizing the Bugs Behind Rejections}
\label{sec:study:bugs}

Across \bench, the bug behind a rejection is usually a genuine developer mistake with a small fix. From the corpus labels we characterize each bug by which layer must change, how the source went wrong, and what the fix changes.

\noindent\textbf{Assigning responsibility.} Genuine bugs in the developer's own code account for 191 of the 235 rejections. We read each rejection's source and fix to assign the layer that must change for the program to pass, shown in Table~\ref{tab:responsibility}. In the other 44, the program's code is already correct: the compiler lowered it into a form the verifier rejects, the environment supplies a wrong type, helper, or metadata, or the verifier rejects a safe program on its own. The rest of this subsection examines the developer bugs.

\input{tables/sec-2-responsibility}
...
\input{tables/sec-2-mechanisms}

\input{figures/sec-2-capacity}
...
\input{tables/sec-2-fanout}

\noindent\textbf{Attributing responsibility.} The message also hides which layer is at fault. The message carries no field for the responsible layer. A source bug, a compiler-lowering rejection, and an environment issue can all print the same line. Figure~\ref{fig:packet} pairs a source bug and a compiler-lowering rejection under one such message. The source bug is fixed in the program. The compiler-lowering rejection is fixed by steering the compiler's code generation. Reading the wrong layer is costly: a developer who mistakes a compiler-lowering rejection for a source bug edits correct source, chasing a fix that belongs in the compiler.

\input{figures/sec-2-packet}
