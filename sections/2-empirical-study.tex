...
We describe how a developer reads a rejection (\S\ref{sec:study:verifier}), assemble \bench, a corpus of real rejections (\S\ref{sec:study:corpus}), characterize the rejections in it (\S\ref{sec:study:bugs}), and measure the log's localization gap (\S\ref{sec:study:gap}).
% 我们讲开发者怎么读一个 rejection（\S\ref{sec:study:verifier}）、构建真实 rejection 语料 \bench（\S\ref{sec:study:corpus}）、刻画其中的 rejection（\S\ref{sec:study:bugs}），并度量这份 log 的定位 gap（\S\ref{sec:study:gap}）。
...
\subsection{Reproducing Real Rejections in \bench}
\label{sec:study:corpus}

To study rejections that developers actually hit, we assemble \bench, a corpus of real verifier rejections reproduced under one fixed toolchain.
% 为研究开发者真正会遇到的 rejection，我们构建 \bench：一份在固定 toolchain 下复现的真实 verifier rejection 语料。
The corpus draws on 936 candidate reports from four sources: Stack Overflow questions, GitHub issues, GitHub fix commits, and kernel selftests.
% 语料取自四个来源的 936 个候选报告：Stack Overflow 提问、GitHub issue、GitHub 修复 commit、内核 selftest。
We rebuild each candidate and load it into kernel 6.15.11 with clang 18 at log level 2, and 235 of the 936 reject under this build and form \bench.
% 我们重建每个候选并加载进 kernel 6.15.11 + clang 18（log level 2），936 个里有 235 个在这套 build 下被拒，构成 \bench。
...
Each \bench case carries the faulty source and the developer's own fix, both from its report.
% 每个 \bench case 都带出错源码和开发者自己的 fix，二者都来自其报告。
...
We characterize each \bench rejection by where its repair lands.
% 我们按每个 \bench rejection 的修复落在哪里来刻画它。
