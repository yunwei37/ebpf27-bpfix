...
Section~\ref{sec:study} shows that a terminal verifier message leaves three parts of a repair unresolved: the bug, theresponsible layer, and the fix site. \sys does not attempt to infer all three directly from the terminal string. Instead, it constructs a verifier-level explanation: it recovers the safety fact required at the rejected operation, traces how the verifier-visible evidence for that fact evolved, and maps the resulting evidence to repair-relevant locations and actions.

We distinguish three locations throughout the design. The \emph{rejection site} is the operation where verification stops. A \emph{proof-break witness} is a state transition after which the printed verifier state no longer supports a required fact. The \emph{repair site} is the source location ultimately changed by the developer. \sys recovers the rejection site and, when the trace exposes one, a proof-break witness. It uses these locations to narrow the repair without assuming that either must coincide with the eventual source edit. Likewise, facts reconstructed from verifier states form the evidence-backed core of the diagnostic, whereas responsibility attribution remains a separate, lower-confidence inference.


\subsection{Architecture and Data Flow}
\label{sec:design-architecture}

Figure~\ref{fig:design-overview} organizes \sys into two layers. The first layer, \emph{verifier-evidence
reconstruction}, consumes an existing rejection log. It isolates the verifier output from surrounding build and loader messages, anchors the rejected instruction, instantiates the safety fact required at that instruction, and follows that fact through the printed abstract states. The result is a compact
evidence record containing the required proof and, when observable, the locations where it was established, lost, and finally required.

The second layer, \emph{diagnostic synthesis}, translates this record into repair guidance. It maps verifier instructions to source annotations when the log provides them, assigns span precision, applies ordered evidence rules to refine the likely failure class, and derives a next action from the missing
proof. The evaluated artifact renders the record as a compiler-style diagnostic and serializes the same evidence through a versioned JSON contract.

The verifier log is the only required input. \sys neither modifies the kernel nor re-runs the verifier, and it constructs no independent safety model. Missing source annotations reduce localization precision but do not prevent the log-only analysis from producing a diagnostic.

\subsection{Reconstructing the Verifier-Visible Proof}
\label{sec:design-proof}

The first analysis task is to determine which fact to follow. A verbose trace may contain hundreds of register and stack states, whereas the terminal message identifies only the operation whose safety precondition could not be proved. \sys uses the message to propose a proof family, such as pointer
provenance, packet bounds, scalar range, non-nullness, stack initialization, or reference lifetime. It then binds that family to concrete values recovered near the rejected instruction, including the affected register, access size, offset, or resource identifier. The terminal message therefore seeds the analysis; it does not determine the final diagnosis.

For the running example, the terminal message \texttt{R5 invalid mem access 'scalar'} suggests a
pointer-provenance failure. From the rejected load and its nearby state, \sys instantiates the required proof as follows: \texttt{R5} must remain a verifier-recognized, in-bounds packet pointer at instruction 37. This identifies both the object being tracked and the property that must hold at its use.

A proof-family-specific tracker then scans the available states. Pointer trackers follow register types, identities, offsets, and accessible ranges; scalar trackers follow known bounds; and resource trackers follow acquisition and release state. The trackers emit up to three event roles. An \emph{established} event records a state containing the required fact. A \emph{lost} event records a later transition
after which the fact is no longer visible. A \emph{rejected} event records the operation that requires the absent fact.

In Figure~\ref{fig:design-overview}, an earlier state represents \texttt{R5} as \texttt{pkt(off=34,r=42)}, a later state represents it as scalar value \texttt{40}, and the final load
attempts to dereference it. Connecting these events distinguishes a proof that was never established from one that was established and subsequently lost. The former suggests introducing a missing check or initialization. The latter suggests preserving verifier-visible evidence across arithmetic, control-flow merging, or lowering, or re-establishing it immediately before use.

The reported loss remains a witness to a verifier-visible transition, not a claim about the smallest causal instruction set or the exact line that a patch must edit. \sys follows the states present in one rejection log; it does not enumerate unprinted paths or replay the verifier. This boundary keeps the
localization faithful to the evidence available to the developer.

\subsection{Evidence-Aware Diagnostic Synthesis}
\label{sec:design-synthesis}

\sys projects the reconstructed lifecycle into a multi-span diagnostic. The rejected operation becomes the primary span, while proof-establishment and proof-loss events become related spans when instruction or source locations are available. Each span carries its localization precision, so an exact program
counter, a nearby source annotation, and a terminal-line fallback are not presented as equivalent locations.
